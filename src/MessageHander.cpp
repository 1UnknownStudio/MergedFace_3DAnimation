...
C_UDPBase::C_UDPBase(const int port, const std::string ip_address, UDPTYPE cmd) :Port(port), IPAddress(ip_address)
{
	int i_result = 0;
	//初始化Socket
...
C_UDPSandG::C_UDPSandG(const int port, const std::string ip_address, int buf_data_len, UDPTYPE type) :C_UDPBase(port, ip_address, type), BufLength(buf_data_len)
{
