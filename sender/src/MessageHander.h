...
public:
	...
	C_UDPBase(const int port, const std::string ip_address, UDPTYPE cmd);//"Any" 代表任何IP地址
...
public:
	char* buffer;
	int BufLength;
	C_UDPSandG(const int port, const std::string ip_address, int buf_data_len, UDPTYPE type);
